...
class Camera(ABC):
    ...
    def define_k_matrix(self):
        """
        This K matrix for pinhole model projection is the traditional K-affine matrix which
        transforms homogeneous coordinates to pixel. However, for spherical and equirectangular
        cameras this Affine matrix transform from spherical coordinates (in Equirectangular images) to
        pixels coordinates instead.\n
        pinhole: dot(K, [x, y, 1].T )= [u, v, 1].T\n
        spherical: dot(K, [theta, phi, 1].T )= [u, v, 1].T\n
        NOTE: The Kinv matrix is also defined here
        """
        raise NotImplementedError
