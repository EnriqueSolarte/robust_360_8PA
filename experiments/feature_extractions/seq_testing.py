...
def eval_camera_pose(tracker, cam_gt, output_dir, file):
    ...
    fig.add_trace(
        go.Scatter(
            x=matches[0, :, 0], y=matches[0, :, 1], mode='markers',
            name='Ref'))
    fig.add_trace(
        go.Scatter(
            x=matches[1, :, 0], y=matches[1, :, 1], mode='markers',
            name='Key'))
    fig['layout']['xaxis'].update(
        title='', range=[0, 1024], dtick=128, autorange=False)
    fig.update_xaxes(
        ticktext=list(range(-180, 180 + 1, 45)),
        tickvals=list(range(0, 1024 + 1, 128)),
    )
    fig['layout']['yaxis'].update(
        title='', range=[0, 512], dtick=128, autorange=False)
    fig.update_yaxes(
