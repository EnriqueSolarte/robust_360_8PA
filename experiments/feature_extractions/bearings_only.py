...
from analysis.sequence_of_frames.based_on_bearings_only import plot, run_sequence, save_info
...
if __name__ == '__main__':
    ...
    for _ in range(n):
        ...
        tmp = run_sequence(**scene_settings,
                           **features_setting,
                           **ransac_parm
                           )
        tmps.append(tmp)
    ...
    mean_error["results"]["kf"] = list(range(len(tmps[0]["results"]["8pa_error_rot"])))
    ...
    for i in range(len(tmps[0]["results"]["8pa_error_rot"])):
        mean_error["results"]["8pa_error_rot"].append(
            np.mean([tmps[j]["results"]["8pa_error_rot"][i] for j in range(n)]))
        mean_error["results"]["norm_8pa_error_rot"].append(
            np.mean([tmps[j]["results"]["norm_8pa_error_rot"][i] for j in range(n)]))
        mean_error["results"]["opt_res_error_rot"].append(
            np.mean([tmps[j]["results"]["opt_res_error_rot"][i] for j in range(n)]))

        mean_error["results"]["8pa_error_tran"].append(
            np.mean([tmps[j]["results"]["8pa_error_tran"][i] for j in range(n)]))
        mean_error["results"]["norm_8pa_error_tran"].append(
            np.mean([tmps[j]["results"]["norm_8pa_error_tran"][i] for j in range(n)]))
        mean_error["results"]["opt_res_error_tran"].append(
            np.mean([tmps[j]["results"]["opt_res_error_tran"][i] for j in range(n)]))

    plot(**mean_error)
    save_info(**mean_error)
