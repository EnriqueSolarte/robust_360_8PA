...
def get_file_name(**kwargs):
    ...
    filename += "_grid_" + str(kwargs["grid"][0]) + "." + str(
        kwargs["grid"][1]) + "." + str(kwargs["grid"][2])
...
def eval_function(**kwargs):
    ...
    print("Using RANSAC")
    print("error_rot: {}    error_tran: {}".format(error_ransac[0],
                                                   error_ransac[1]))
    ...
    print("error_rot: {}    error_tran: {}".format(error_no_ransac[0],
                                                   error_no_ransac[1]))
    ...
    for i in range(len(v) * len(v)):
        ...
        cam_hat = g8p_norm.recover_pose_from_e(e_hat, bearings_kf_all,
                                               bearings_frm_all)
        ...
        bearings_frm_norm_inliers, T2 = g8p_norm.normalizer(
            bearings_frm_inliers, S, K)
        e_hat = g8p.compute_essential_matrix(bearings_kf_norm_inliers,
                                             bearings_frm_norm_inliers)
        e_hat = np.dot(T1.T, np.dot(e_hat, T2))
        cam_hat = g8p_norm.recover_pose_from_e(e_hat, bearings_kf_inliers,
                                               bearings_frm_inliers)
        error_cam = evaluate_error_in_transformation(cam_hat, cam_gt)
