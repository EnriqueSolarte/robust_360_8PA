...
def plot_histograms_of_errors(scene,
    ...
    if r:
        for res in ress:
            dt = pd.read_csv("../report/{}/{}/{}/{}_{}x{}.csv".format(
            ...
            print("Q2-ours:{}- {}".format(np.median(data[:, 2:4], axis=0),
                                          len(data[:, 0])))
            ...
            print("Q2-8PA:{}-  {}".format(np.median(data[:, 0:2], axis=0),
                                          len(data[:, 0])))
            print("Q3-8PA:{}-  {}".format(
...
def plot_frame_errors(filename):
    ...
    # ! Ours' method
    print("Q1-ours:{}- {}".format(np.quantile(data[:, 2:4], 0.25, axis=0),
                                  len(data[:, 0])))
    print("Q2-ours:{}- {}".format(np.median(data[:, 2:4], axis=0),
                                  len(data[:, 0])))
    print("Q3-ours:{}- {}".format(np.quantile(data[:, 2:4], 0.75, axis=0),
                                  len(data[:, 0])))
    print(
        "====================================================================="
    )
    # ! 8PA
    print("Q1-8PA:{}-  {}".format(np.quantile(data[:, 0:2], 0.25, axis=0),
                                  len(data[:, 0])))
    print("Q2-8PA:{}-  {}".format(np.median(data[:, 0:2], axis=0),
                                  len(data[:, 0])))
    print("Q3-8PA:{}-  {}".format(np.quantile(data[:, 0:2], 0.75, axis=0),
                                  len(data[:, 0])))
    print(
        "====================================================================="
    )
...
def plot_median_errors(scene,
    ...
    if r:
        ...
        plt.title("{}-{}-{}-{}-{}".format(scene, str(idx_frame),
                                          "mc" if motion_constraint else "!mc",
                                          opt_version, 500))
        plt.subplots_adjust(left=0.125, right=0.955, bottom=0.230, top=0.865)
        plt.savefig("../report/{}/{}/{}/{}_{}_fov.png".format(
            scene, str(idx_frame), "mc" if motion_constraint else "!mc", 500,
...
if __name__ == '__main__':
    ...
    plot_median_errors(motion_constraint=True,
                       opt_version="v2",
                       scene="1LXtFkjw3qL",
                       idx_frame=0,
                       r=False)
