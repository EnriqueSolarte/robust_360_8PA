...
def plot_histograms_of_errors(scene, idx_frame, opt_version, motion_constraint, r=True):
    ...
    if r:
        for res in ress:
            dt = pd.read_csv(
                "../report/{}/{}/{}/{}_{}x{}.csv".format(scene, str(idx_frame), "mc" if motion_constraint else "~mc",
                                                         opt_version, str(res[0]), str(res[1])), delimiter=" ")
            ...
            plt.figure()
            plt.subplot(1, 2, 1)
            plt.hist(data[:, 0], density=True, bins=data.shape[0] // 10, label=headers[0])
            ...
            plt.legend()
            plt.title(
                "{}-{}-{}-{}-{}x{}".format(scene, str(idx_frame), "mc" if motion_constraint else "~mc", opt_version,
                                           str(res[0]), str(res[1])))
            plt.show()
    else:
        ...
        for noise in noises:
            dt = pd.read_csv(
                "../report/{}/{}/{}/{}/{}_{}x{}.csv".format(scene, str(idx_frame), "mc" if motion_constraint else "~mc",
                                                            noise, opt_version, str(ress[1][0]), str(ress[1][1])),
                delimiter=" ")
            headers = dt.columns.values
            data = dt.values
            print("=====================================================================")
            ...
            plt.legend()
            plt.title(
                "{}-{}-{}-{}-{}-{}x{}".format(scene, str(idx_frame), "mc" if motion_constraint else "~mc",
                                              noise, opt_version, str(ress[1][0]), str(ress[1][1])))
            plt.show()
...
def plot_median_errors(scene, idx_frame, opt_version, motion_constraint, r=True):
    ress = [(54.4, 37.8), (65.5, 46.4), (195, 195), (360, 180)]
    # ress = [(3.44, 5.15), (16.1, 23.9), (27.0, 39.6), (54.4, 37.8), (65.5, 46.4), (81.2, 102.7), (195, 195), (360, 180)]
    ...
    if r:
        ...
        plt.title("{}-{}-{}-{}-{}".format(scene, str(idx_frame), "mc" if motion_constraint else "~mc", opt_version,
                                             500))
        plt.subplots_adjust(left=0.125, right=0.955, bottom=0.230, top=0.865)
        plt.savefig(
            "../report/{}/{}/{}/{}_{}_fov.png".format(scene, str(idx_frame), "mc" if motion_constraint else "~mc", 500,
                                                      opt_version, str(ress[1][0]), str(ress[1][1])))
        plt.show()
    else:
        noises = [500, 1000, 2000, 10000]
        ...
        plt.title("{}-{}-{}-{}-{}x{}".format(scene, str(idx_frame), "mc" if motion_constraint else "~mc", opt_version,
                                             str(ress[1][0]), str(ress[1][1])))
        plt.subplots_adjust(left=0.215, right=0.780, bottom=0.195, top=0.885)
        plt.savefig(
            "../report/{}/{}/{}/{}_{}x{}_noise.png".format(scene, str(idx_frame), "mc" if motion_constraint else "~mc",
                                                           opt_version, str(ress[1][0]), str(ress[1][1])))
        plt.show()
...
if __name__ == '__main__':
    ...
    plot_median_errors(motion_constraint=True, opt_version="v0", scene="1LXtFkjw3qL", idx_frame=0, r=True)
